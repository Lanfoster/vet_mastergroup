define(['jquery', 'Backbone','underscore', 'transit', 'snap' ], function( $, Backbone, _, transit, Snap )
{
  ...
  script =
  {
    ...
    init: function( container )
    {
      ...
      container.find('.content, .content ul li, .popBtn').css( { opacity : 0, y : 40 } );
    ...
    show: function( container )
    {
      ...
      container.find('h1, .content').each(function(){ next.push([this, 1]) });
      next.push([1]);
      container.find('.content ul li, .popBtn').each(function(){ next.push([this, 1]) });
